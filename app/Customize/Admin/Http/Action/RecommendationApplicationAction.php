...
class RecommendationApplicationAction extends Action
{
    public static function list(array $param)
    {
        $order = parse_order($param['order']);
        $res = RecommendationApplication::list($param , $order , config('app.limit'));
    ...
    public static function updateStatus(array $param)
    {
        ...
        if (!in_array($param['status'] , config('business.recommendation_application_status_range'))) {
            return self::error('不支持的状态值');
        }
        $res = RecommendationApplication::updateByIds($id_list , array_unit($param , [
            'status'
        ]));
        return self::success($res);
    }
