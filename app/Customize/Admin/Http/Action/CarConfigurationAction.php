...
class CarConfigurationAction extends Action
{
    public static function list(array $param)
    {
        $order = parse_order($param['order']);
        $res = CarConfiguration::list($param , $order , config('app.limit'));
    ...
    public static function add(array $param)
    {
        $validator = Validator::make($param , [
            'name' => 'required' ,
            'car_configuration_group_id' => 'required' ,
        ] , [
            'name.required' => 'name 尚未提供' ,
            'car_configuration_group_id.required' => 'car_configuration_group_id 尚未提供' ,
        ]);
        if ($validator->fails()) {
            return self::error($validator);
        }
        $param['weight'] = $param['weight'] != '' ? intval($param['weight']) : config('app.weight');
        $id = CarConfiguration::insertGetId(array_unit($param , [
            'name' ,
    ...
    public static function edit(array $param)
    {
        ...
        $m = CarConfiguration::find($param['id']);
        if (empty($m)) {
            return self::error('未找到 id 对应记录');
        }
        $param['weight'] = $param['weight'] == '' ? $m->weight : $param['weight'];
        CarConfiguration::updateById($param['id'] , array_unit($param , [
            'name' ,
            'car_configuration_group_id' ,
            'desc' ,
            'weight'
    ...
    public static function del(array $param)
    {
        ...
        $res = CarConfiguration::destroy($id_list);
        return self::success($res);
    }

    public static function detail($id)
    {
        $res = CarConfiguration::findById($id);
        if (empty($res)) {
            return self::error("未找到 id 对应记录");
        }
        return self::success($res);
