...
class CarConfigurationGroupAction extends Action
{
    public static function list(array $param)
    {
        $order = parse_order($param['order']);
        $res = CarConfigurationGroup::list($param , $order , config('app.limit'));
    ...
    public static function add(array $param)
    {
        ...
        $id = CarConfigurationGroup::insertGetId(array_unit($param , [
            'name' ,
            'weight'
        ]));
    ...
    public static function edit(array $param)
    {
        ...
        $m = CarConfigurationGroup::find($param['id']);
        if (empty($m)) {
            return self::error('未找到 id 对应记录');
        }
        $param['weight'] = $param['weight'] == '' ? $m->weight : $param['weight'];
        CarConfigurationGroup::updateById($param['id'] , array_unit($param , [
            'name' ,
            'weight'
        ]));
    ...
    public static function del(array $param)
    {
        ...
        $res = CarConfigurationGroup::destroy($id_list);
        return self::success($res);
    }

    public static function detail($id)
    {
        $res = CarConfigurationGroup::find($id);
        if (empty($res)) {
            return self::error("未找到 id 对应记录");
        }
        return self::success($res);
    ...
    {
        $res = CarConfigurationGroup::getAll();
        return self::success($res);
    }
}