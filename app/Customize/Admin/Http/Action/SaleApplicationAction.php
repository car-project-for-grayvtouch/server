...
class SaleApplicationAction extends Action
{
    ...
    public static function updateStatus(array $param)
    {
        ...
        if (!in_array($param['status'] , config('business.sale_application_status_range'))) {
            return self::error('不支持的状态值');
        }
        $res = SaleApplication::updateByIds($id_list , array_unit($param , [
            'status'
        ]));
        return self::success($res);
    }
