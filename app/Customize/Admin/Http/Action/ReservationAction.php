...
class ReservationAction extends Action
{
    public static function list(array $param)
    {
        ...
        $res = Reservation::list($param , $order , config('app.limit'));
    ...
    public static function updateStatus(array $param)
    {
        ...
        if (!in_array($param['status'] , config('business.reservation_application_status_range'))) {
            return self::error('不支持的状态值');
        }
        $res = Reservation::updateByIds($id_list , array_unit($param , [
            'status'
        ]));
        return self::success($res);
    }
