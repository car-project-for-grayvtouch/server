...
class CarSeriesGroupAction extends Action
{
    public static function list(array $param)
    {
        $order = parse_order($param['order']);
        $res = CarSeriesGroup::list($param , $order , config('app.limit'));
    ...
    public static function add(array $param)
    {
        ...
        $id = CarSeriesGroup::insertGetId(array_unit($param , [
            'name' ,
            'weight'
        ]));
    ...
    public static function edit(array $param)
    {
        ...
        $m = CarSeriesGroup::find($param['id']);
        if (empty($m)) {
            return self::error('未找到 id 对应记录');
        }
        $param['weight'] = $param['weight'] == '' ? $m->weight : $param['weight'];
        CarSeriesGroup::updateById($param['id'] , array_unit($param , [
            'name' ,
            'weight'
        ]));
    ...
    public static function del(array $param)
    {
        ...
        $res = CarSeriesGroup::destroy($id_list);
        return self::success($res);
    }

    public static function detail($id)
    {
        $res = CarSeriesGroup::find($id);
        if (empty($res)) {
            return self::error("未找到 id 对应记录");
        }
        return self::success($res);
